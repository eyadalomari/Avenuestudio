                ...
                <div class="form-group row mt-3">
                    ...
                    <div class="col-10">
                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name_{{ $language->language_id }}"
                            name="name[{{ $language->language_id }}]" placeholder="Enter {{ $language->name }} name" value="{{ old('name['.$language->language_id.']', $role->labels[$language->language_id]->name ?? '') }}">
                        @error('name')
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-10">
                    <input type="text" class="form-control @error('code') is-invalid @enderror" id="code"
                        name="code" placeholder="Enter Code" value="{{ old('code', $role->code ?? '') }}">
                    @error('code')
                        <div class="invalid-feedback" style="display: block">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group row mt-3">
                ...
                <div class="col-10">
                    <input type="text" class="form-control @error('sort') is-invalid @enderror" id="sort"
                        name="sort" placeholder="Enter Sort" value="{{ old('sort', $role->sort ?? '') }}">
                    @error('sort')
                        <div class="invalid-feedback" style="display: block">{{ $message }}</div>
                    @enderror
                </div>
